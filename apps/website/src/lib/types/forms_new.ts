...
function isZodEnumSchema(schema: FieldSchema): schema is z.ZodEnum<[string, ...string[]]> {
		...
		switch (field.type) {
						...
						if (field.validator.format) {
							switch (field.validator.format) {
								...
								case 'url':
									...
									break;
								case 'phone':
									fieldSchema = fieldSchema.regex(/^\+?[1-9]\d{1,14}$/); // Simple E.164 format
									break;
							}
						}
					}
			...
			case 'dropdown':
				if (field.type === 'radio' && field.options && field.options.length > 0) {
					...
					fieldSchema = z.enum(optionLabels as [string, ...string[]]);
				} else {
					fieldSchema = z.string();
				}
				break;
