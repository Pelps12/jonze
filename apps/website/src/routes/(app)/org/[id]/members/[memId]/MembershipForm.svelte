					...
					<Command.Group>
						{#each plans as plan}
							...
							>
								{plan.name}
								<Check
									class={cn('ml-auto h-4 w-4', plan.id !== $formData.planId && 'text-transparent')}
								/>
							</Command.Item>
						{/each}
					</Command.Group>
		...
		<Popover.Root bind:open={open['provider']} let:ids>
			<Form.Control let:attrs>
				...
				<Popover.Trigger
					...
					{...attrs}
				>
					{providerEnum.enumValues.find((f) => f === $formData.provider) ?? 'Select form'}
					<ChevronsUpDown class="ml-2 h-4 w-4 shrink-0 opacity-50" />
				</Popover.Trigger>
			...
			<Popover.Content class="w-[200px] p-0">
				<Command.Root>
					<Command.Input autofocus placeholder="Search provider..." class="h-9" />
					<Command.Group>
