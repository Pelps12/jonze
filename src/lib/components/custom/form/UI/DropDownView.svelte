...
<FieldWrapper handleDelete={handleDelete} handleSave={handleSave}>
    <div class="flex flex-col gap-2 relative">
        <div class="flex flex-col space-y-1.5 ">
            ...
            <Input id="name" placeholder="Label" value={temporaryEdit.label} 
                on:input={(e) =>handleChange({
          ...
          <div class="flex flex-col space-y-1.5">
                ...
                {#each temporaryEdit.options as option, index}
                    <div class="flex justify-between items-center gap-2">
                        <Input id={index.toString()} value={option.label} on:input={(e) => handleChangeOption(index, {
                            label: e.target?.value
                        })}/>
                        <Button variant="destructive" size="icon" class="h-7 w-7" on:click={() => handleItemDelete(index)}>
                            <Trash class="h-4 w-4"/>
                        </Button>
                    </div>
